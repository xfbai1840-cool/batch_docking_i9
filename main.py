...
def is_safe_for_3d(mol):
    ...
    if bad_atoms:
        return False, f"异常元素: {','.join(set(bad_atoms))}"
    
    heavy_atoms = mol.GetNumHeavyAtoms()
    if heavy_atoms > 80:
        return False, f"重原子过多({heavy_atoms})"
    return True, "安全"
...
def generate_3d_mol_fast(mol):
    try:
        # 🚀 换用更快的 ETKDGv2 算法，大幅缩短生成时间
        params = AllChem.ETKDGv2()
        params.useRandomCoords = True
        params.maxIterations = 200 # 减少迭代次数，加快速度
...
def process_single_smiles(cas_name, smiles, output_dir, receptor_file, vina_exe):
    try:
        safe_name = "".join(c for c in cas_name if c.isalnum() or c in ('_', '-')).rstrip()
        ...
        if mol is None: return {'CAS': cas_name, 'Score': None, 'Status': 'SMILES解析失败'}

        frags = list(Chem.GetMolFrags(mol, asMols=True))
        if not frags: return {'CAS': cas_name, 'Score': None, 'Status': '碎片异常'}
        mol = max(frags, key=lambda m: m.GetNumAtoms())

        # 2. 检查与 3D 生成
        mol = Chem.AddHs(mol)
        safe, reason = is_safe_for_3d(mol)
        if not safe: return {'CAS': cas_name, 'Score': None, 'Status': reason}

        if not generate_3d_mol_fast(mol):
            return {'CAS': cas_name, 'Score': None, 'Status': '3D构象生成失败'}

        # 3. Meeko 转换
        try:
            preparator = MoleculePreparation()
            setups = preparator.prepare(mol)
            if not setups: return {'CAS': cas_name, 'Score': None, 'Status': 'Meeko失败'}
            pdbqt_string, is_ok, _ = PDBQTWriterLegacy.write_string(setups[0])
            if not is_ok: return {'CAS': cas_name, 'Score': None, 'Status': 'PDBQT转换异常'}
        except Exception: return {'CAS': cas_name, 'Score': None, 'Status': 'Meeko崩溃'}
        ...
        try:
            ...
            return {'CAS': cas_name, 'Score': None, 'Status': 'Vina引擎运算报错'}
        finally:
            # 无论成功失败，阅后即焚临时输入文件
            if os.path.exists(ligand_pdbqt):
                try: os.remove(ligand_pdbqt)
                except: pass

        if best_score is None:
            if os.path.exists(output_pdbqt):
                try: os.remove(output_pdbqt)
                except: pass
            return {'CAS': cas_name, 'Score': None, 'Status': '无有效构象'}

        return {'CAS': cas_name, 'Score': best_score, 'Status': '成功'}

    except Exception:
        return {'CAS': cas_name, 'Score': None, 'Status': '未知异常终止'}
...
def main():
    ...
    print("=" * 60)
    print("🚀 i9-14900 满血并发筛选引擎 (Vina HTVS)")
    print("=" * 60)
    ...
    result_csv = filedialog.asksaveasfilename(
        title="4/4 保存排行榜结果", defaultextension=".csv", initialfile="HTVS_Docking_Results.csv"
    )
    ...
    # 🚀 彻底修复瓶颈：在外部仅开启【一个】进程池，直接压入所有任务，让操作系统自行榨干 CPU
    with ProcessPoolExecutor(max_workers=MAX_WORKERS) as executor:
        futures = {
            executor.submit(process_single_smiles, cas, smiles, output_dir, receptor_file, vina_exe): cas
        ...
        for future in as_completed(futures):
            ...
            try:
                ...
                res = {'CAS': futures[future], 'Score': None, 'Status': '进程池调度异常'}
    ...
    with open(result_csv, 'w', newline='', encoding='utf-8-sig') as f:
        writer = csv.DictWriter(f, fieldnames=['CAS', 'Score', 'Status'])
        writer.writeheader()
    ...
    print("\n" + "=" * 60)
    print(f"🎉 筛选彻底完成！")
    print(f"⏱️ 总耗时: {elapsed / 60:.2f} 分钟 (平均速度飙升)")
    print(f"✅ 成功对接: {success_count} 个化合物")
    messagebox.showinfo("算力释放成功", f"HTVS 筛选完成！\n成功对接 {success_count}/{total}\n耗时仅 {elapsed/60:.2f} 分钟！")
